...
function Home() {
  ...
  useEffect(() => {
    if (userData.account) {
      const friendsRef = gun.get('usersList')
      ...
      const friendsSubscription = friendsRef.map().on((friend, id) => {
        if (friend) {
          setUserData(prev => {
            ...
            const friendExists = Array.isArray(prev.friends) && 
              prev.friends.some(f => f.id === id);
            
            if (!friendExists) {
              
              const updatedFriends = Array.isArray(prev.friends) ? 
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-6">
          ...
          <div className="space-y-2">
            <p><strong>Connected Address:</strong> {userData.account}</p>
